const {DWH_MTD_ACC_EXPENSE} = require('../queues/queues')
...
module.exports = () => {
    DWH_MTD_ACC_EXPENSE.process(async(job, done) => {
        try{
            ...
            const fileName = moment().format('YYYYMMDDHHmmss')+'expense_daily_mtd_accrual_report.xlsx'
    ...
    DWH_MTD_ACC_EXPENSE.on('active', async(job) => {
        ...
        if(!isJobExists) {
            const report = await reportService.findReport({
                report_name: 'daily_mtd_accrual_expense'
            })
    ...
    DWH_MTD_ACC_EXPENSE.on('completed', async(job) => {
    ...
    DWH_MTD_ACC_EXPENSE.on('failed', async(job,err) => {
        console.log(err)
        console.log(job.id)
        await reportService.updateReportLog({
            filter:{
