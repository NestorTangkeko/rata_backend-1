const {DWH_MTD_ACC_REVENUE} = require('../queues/queues')
...
module.exports = () => {
    DWH_MTD_ACC_REVENUE.process(async(job,done) => {
        try{
            ...
            const start_trip_date = moment().startOf('month').format('YYYY-MM-DD');
            const end_trip_date = moment.format('YYYY-MM-DD');

            const data = await podReportService.joinedHandedOverInvoicesRange(start_trip_date, end_trip_date);
            const vehicle_types = await podReportService.getKronosVehicleTypes();

            const draftBill = await podReportService.podSell({
                data,
                from: start_trip_date,
                to: end_trip_date
            })
            ...
            const fileName = moment().format('YYYYMMDDHHmmss')+'revenue_daily_mtd_accrual_report.xlsx'
            ...
            await podReportExcelService.podAccrualTemplate({
                ...
                filePath,
                type:           'SELL',
                from:           moment(start_trip_date).format('MMMM DD, YYYY'),
                to:             moment(end_trip_date).format('MMMM DD, YYYY'),
            })
            ...
            await job.progress('completed')
                ...
                filePath,
                fileName,
                transaction_date: end_trip_date
            });
    ...
    DWH_MTD_ACC_REVENUE.on('active', async(job) => {
        ...
        if(!isJobExists) {
            const report = await reportService.findReport({
                report_name: 'daily_mtd_accrual_revenue'
            })
    ...
    DWH_MTD_ACC_REVENUE.on('completed', async(job) => {
        ...
        console.log('Daily Month-to-date Accrual Revenue Report Done')
        console.log(job.id)
    })

    DWH_MTD_ACC_REVENUE.on('failed', async(job,err) => {
        console.log(err)
        await reportService.updateReportLog({
            filter:{
                id: job.id,
